...
pub(super) fn local_hello_capability_view_from_config(
    config: &mct_daemon::MctDaemonConfig,
    state_path: &Path,
    children_dir: &Path,
...
pub(super) fn refresh_remote_surfaces_from_admitted_hello_response(
    state_path: &Path,
...
#[derive(Clone)]
pub(super) struct ResidentLedgerWriter {
    pub(super) sender: tokio::sync::mpsc::Sender<ResidentLedgerWrite>,
}
...
impl ResidentLedgerWriter {
    pub(super) fn spawn(path: PathBuf) -> Result<Self> {
...
#[derive(Clone, Debug)]
pub(super) struct ResidentRequestPayload {
    pub(super) inline_payload: Option<Vec<u8>>,
    pub(super) allow_local_content_addressed_blob: bool,
}

impl ResidentRequestPayload {
    pub(super) fn remote(inline_payload: Option<Vec<u8>>) -> Self {
        Self {
            inline_payload,
...
#[derive(Clone, Debug)]
pub(super) struct ResidentExecutionReport {
    pub(super) result: MctResult,
    pub(super) observations: Vec<MctObservation>,
    pub(super) inline_result_payload: Option<Vec<u8>>,
}
...
pub(super) struct ResidentPayloadResolutionFailure {
...
pub(super) fn execute_resident_process_child(
...
pub(super) fn execute_resident_wit_child(
    execution: ResidentChildExecution,
    request: &MctCallProtocolRequest,
    inline_payload: Option<&[u8]>,
) -> Result<ResidentExecutionReport> {
