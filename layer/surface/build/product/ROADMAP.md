...
- [ ] Make the exact supported `folder-watch-actor@0.1.0` and `watch-null-sink@0.1.0` artifacts available to MCT CI so their declared WIT exports can be invoked alongside the real `slate-manager@0.2.0` acquisition-to-execution fixture. Daily-Driver Slice 3 proves Slate from immutable acquisition through supervised execution, and Replacement Slice 4A now proves durable temporal trigger authority, restart recovery, and governed WIT invocation. Replacement Slice 4B still owns Watch authority, child call-out, legacy ABI narrowing, and both remaining real fixtures before the full three-fixture `patinaMother` replacement test can close.
