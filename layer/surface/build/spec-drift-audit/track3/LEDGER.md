...
### Release discipline and daemon upgrade extension

| Invariant / obligation | Status | Evidence |
|---|---|---|
| `MctOperationalSelfObservation.LifecycleActionAttemptsAreObserved` — daemon upgrade success, denial, and partial failure | COVERED | `mct_daemon_bin::supervisor_lifecycle::tests::exact_upgrade_approval_composes_shared_replace_and_named_post_verify` orders exact approval, shared stop / `install --replace` / start, bounded post-verification, completion, and no-automatic-rollback failure evidence; `mct_daemon_bin::upgrade::tests::approval_is_only_the_complete_exact_archive_sha256_identity` proves similar version/name/broad confirmation cannot approve. |
| `MctOperationalSelfObservation.StatusAndReadinessAreProjections` — post-upgrade identity | COVERED | `mct_daemon_bin::cli_admin::tests::status_reports_real_resident_snapshot` and the exact upgrade integration project product version, supervisor revision, and executable digest from the running successor without granting authority. |
| `MctOperationalSelfObservation.WriterLossFencesMother` — daemon release publication | COVERED | `mct_daemon::release::acquisition_tests::release_publication_rolls_back_when_terminal_writer_is_lost` proves terminal writer loss removes the state projection and immutable release tree rather than acknowledging unobserved publication. |
| `MctAuthorityMutationOwnership.ResidentOwnsLiveMutations` — daemon release acquisition | COVERED | `mct_daemon::release::acquisition_tests::operator_file_release_acquisition_is_observed_immutable_and_not_child_evidence` plus `mct_daemon_bin::supervisor_lifecycle::tests::daemon_release_upgrade_surface_is_distinct_exact_and_source_closed` prove the owner-authenticated resident path owns live acquisition and emits daemon-specific evidence. |
| `MctAuthorityMutationOwnership.OfflineMutationRequiresExclusiveLedgerOwnership` — daemon release acquisition | COVERED | `mct_daemon::release::acquisition_tests::operator_file_release_acquisition_is_observed_immutable_and_not_child_evidence` exercises the shared offline writer path; writer contention and terminal-writer-loss tests prove no second visible publication escapes ownership. |
| `MctAuthorityMutationOwnership.ConnectedResidentResponseNeverFallsBack` — upgrade acquisition | COVERED | `mct_daemon_bin::supervisor_lifecycle::tests::daemon_release_upgrade_surface_is_distinct_exact_and_source_closed` proves resident routing remains source-closed and a connected response is consumed without a second filesystem/offline attempt. |
| `MctAuthorityMutationOwnership.MutationDecisionIsDurableBeforeEffect` — exact-approved replacement | COVERED | `mct_daemon_bin::supervisor_lifecycle::tests::exact_upgrade_approval_composes_shared_replace_and_named_post_verify` proves `upgrade_approval_admitted` precedes stop and replacement; missing/wrong approval leaves the record, running instance, and executable binding unchanged in the real packaged smoke. |
| Daemon release evidence remains outside Child/Toy authority ontology | COVERED | `mct_daemon::state::tests::schema_v11_keeps_daemon_release_evidence_out_of_child_ontology`; `mct_daemon::release::acquisition_tests::operator_file_release_acquisition_is_observed_immutable_and_not_child_evidence`. |
| `NetworkArtifactAcquisitionAdapter` and credential-bearing release discovery | DEFERRED | R5 update-channel scope; `mct_daemon::release::acquisition_tests::release_source_plan_is_operator_file_only_and_network_adapter_remains_closed` proves no latent network path. |
| Linux/systemd supervision and Linux release signing | DEFERRED | The target adapter rejects unsupported release targets and the production lifecycle remains macOS-only; separate `SystemdUserSupervisorAdapter` and `LinuxReleaseSigningAdapter` specifications are required. |
