...
- **2026-07-12 — cancelled result projection (awaiting adjudication):** Track 3 full-ledger propagation's expected-red probe `cancelled_result_projection_preserves_cancelled_outcome` (captured verbatim in Track 3 `TASKS.md` and removed after the structural stop) caught that the real resident result consumer maps `ResultOutcome::Cancelled` to `CallProtocolOutcome::Failed`. The mismatch is structural: result and reply models include cancellation, but the protocol-evaluation outcome does not. No behavior or wire change was made pending operator disposition.
