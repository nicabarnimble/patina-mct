...
- **2026-07-12 — rejected child artifact observation:** Track 3 full-ledger propagation's `mct_daemon_bin::control::tests::live_registry_sync_observes_artifact_rejection_before_state_effect` caught that strict registry-sync load failures were omitted from the decision observation batch because only successfully loaded children were projected. Fixed in `4e992e6` (`fix(control): complete child lifecycle observations`); rejected artifacts now append typed `ArtifactRejected` facts before the registry state effect.
- **2026-07-12 — cancelled result projection:** Track 3 full-ledger propagation's expected-red `mct_daemon_bin::resident::execution::tests::cancelled_result_projection_preserves_cancelled_outcome` caught that the real resident result consumer mapped `ResultOutcome::Cancelled` to `CallProtocolOutcome::Failed`. Operator-adjudicated Option 1 is fixed in `8565636` (`fix(kernel): preserve cancelled protocol outcomes`): protocol evaluation, caller-safe reply projection, durable idempotent replay, and result observations now preserve cancellation end-to-end while suppressing `route_taken`.
